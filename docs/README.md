# CML · CNMarketLake — 本地可日更的 A 股研究湖

本地可日更、可溯源的金融数据研究底座：多源采集、编排、标准化，交付带溯源、列契约稳定的 Parquet 湖。

CLI 是 `cml`，Python 包是 `cn_market_lake`。推荐：`pip install cn-market-lake` → `cml demo` 试玩，或 `cml config init` 写出配置再跑全量。数据湖根目录默认 `./data/cn-market-lake`。
...
可搜索的在线文档站：[rootsunc.github.io/cn-market-lake](https://rootsunc.github.io/cn-market-lake/)。
...
- [数据源健康度](operations/source-health.md)（`cml sources` + `cml serve /source-health`）
