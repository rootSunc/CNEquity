...
def _probe_tdx(config: Config) -> str:
    from cn_market_lake.adapters.tdx_protocol.client import fetch_daily_bars
...
def _probe_em_push2(config: Config) -> str:
    ...
    """
    from cn_market_lake.adapters.eastmoney.common import ALL_A_FS, PUSH2_CLIST_HOSTS
...
def _probe_em_push2his(config: Config) -> str:
    from cn_market_lake.adapters.eastmoney.common import PUSH2HIS_KLINE_HOSTS
...
def _probe_cninfo(config: Config) -> str:
    ...
    from cn_market_lake.adapters.cninfo.announcements import _CNINFO_URL
...
def _probe_ths_kline(config: Config) -> str:
    from cn_market_lake.adapters.ths.boards import _get
    from cn_market_lake.adapters.ths.stock_bars import _STOCK_KLINE_URL
...
def _probe_ths_pages(config: Config) -> str:
    from cn_market_lake.adapters.ths.boards import _INDUSTRY_URL, _get
...
def _probe_sse(config: Config) -> str:
    from cn_market_lake.adapters.exchange.st_lists import _SSE_HEADERS, SSE_URL, _client
...
def _probe_szse(config: Config) -> str:
    from cn_market_lake.adapters.exchange.st_lists import _SZSE_HEADERS, SZSE_URL, _client
...
def _probe_pboc(config: Config) -> str:
    from cn_market_lake.adapters.pboc._tables import STATS_INDEX, get_text
...
def _probe_nbs(config: Config) -> str:
    from cn_market_lake.adapters.nbs.pmi_release import find_latest_release
...
def run_probes(
    ...
    *,
    ...
    try:
        pkg_version = version("cn-market-lake")
