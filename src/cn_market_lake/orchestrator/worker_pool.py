...
from cn_market_lake.adapters.tdx_protocol.client import fetch_daily_bars, normalize_with_source
...
from cn_market_lake.quality.failover import snapshot_daily_bars_backup
...
def _window_backfill(config: Config, start: date) -> bool:
    ...
    start a backfill ever had. It is not any more: `cml init --since` picks its
...
def _worker_fetch_batch(args: tuple) -> dict[str, Any]:
    ...
    try:
        ...
        if failover_enabled and dataset == "daily_bars" and start < end:
            from cn_market_lake.adapters.eastmoney.bars import fetch_daily_bars as fetch_em_bars
