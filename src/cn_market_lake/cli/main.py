...
from cn_market_lake.config import WaveConfig, load_config, validate_config, write_user_config
...
from cn_market_lake.storage.staging_cleanup import clean_staging

USER_CONFIG = "configs/cn-market-lake.toml"
EXAMPLE_CONFIG = "configs/cn-market-lake.example.toml"
DEFAULT_CONFIG = USER_CONFIG

# `cml init --profile quick`. Three years is the shortest window that still
...
def _progress_logging(quiet: bool = False) -> None:
    ...
    Long fetches were silent until they finished: `cml init` runs for hours and
...
def resolve_config_path(config_path: str) -> Path:
    ...
    if config_path == USER_CONFIG and not path.exists():
        raise click.ClickException(
            ...
            "Run `cml config init` to write one from the packaged example "
...
@click.group()
@click.version_option(package_name="cn-market-lake")
def cli():
    """cn-market-lake — A-share data ingestion CLI."""
...
@click.option(
    "--data-root",
    default="data/cn-market-lake-demo",
    show_default=True,
...
@click.option(
    "--config-out",
    default="configs/cn-market-lake.demo.toml",
    show_default=True,
    help="Where to write the tiny demo config for follow-up `cml query`.",
)
@click.option(
    "--intraday",
    is_flag=True,
...
def demo_cmd(
    ...
    Not a full-market backfill — use `cml init` for that. Requires network access
    to TDX hosts (mainland egress is more reliable overseas).
    """
    from cn_market_lake.cli.demo import run_demo
...
@click.option(
    ...
    "Both fetch every symbol — deepen later with `cml backfill daily_bars`.",
)
@click.option(
    "--since",
    "since_str",
...
def init(
    ...
    if history_start is not None:
        ...
        click.echo(
            ...
            "Deepen later with `cml backfill daily_bars --start <earlier>`."
        )
    ...
    if not resume and not resume_run_id:
        ...
        if incomplete is not None:
            raise click.ClickException(
                ...
                "Use `cml init --resume` or `cml retry --run-id "
...
@click.option(
    "--data-root",
    default=None,
    help="Set [data].root when action=init (default: resolve ./data/cn-market-lake to an absolute path).",
)
def config_cmd(action: str, config_path: str, force: bool, data_root: str | None):
    """Validate or bootstrap configuration.

    ``cml config init`` writes the packaged example TOML (no repo checkout needed).
    On macOS it also forces ``orchestrator.workers = 1``.
    ``cml config validate`` checks an existing file.
    """
    if action == "init":
        ...
        click.echo("data.root is absolute; edit if needed, then: cml config validate && cml init")
        return
...
def doctor(config_path: str, as_json: bool):
    ...
    from cn_market_lake.diagnostics.report import build_report
...
def stale_fetch_steps(cfg, anchor: date) -> list[str]:
    ...
    Freshness is judged exactly as ``cml status --datasets`` judges it, so the
    two cannot disagree about what is behind.

    Derived datasets are excluded: they are recomputed by ``cml derive`` from
    ...
    """
    # Steps are registered by the module-level `import cn_market_lake.steps`.
    from cn_market_lake.domain.datasets import DATASETS, is_stale
...
def run_catchup(
    ...
    """
    from cn_market_lake.steps.common import is_trading_day, list_trading_dates
...
# `instruments`, and the block that holds it. `cml backfill --symbols` and the
...
def _guard_history_horizon(dataset: str, start: date | None) -> None:
    ...
    it returns none — so without this an ``cml backfill minute_bars --start
...
def _finish_backfill_run(engine, result: dict) -> dict:
    ...
    # batch is what later lets `cml clean` release this run's staging.
...
def _run_backfill(cfg, dataset: str, start: date | None, end: date | None) -> dict:
    """Backfill one window, dispatching exactly as `cml backfill` does.

    Shared so `cml verify --repair` cannot drift into a second, subtly
...
def _backfill_once(cfg, dataset: str) -> dict:
    ...
    # leaves status=success with no compact batch, and `cml clean` cannot reclaim
...
def _backfill_symbol_chunked(cfg, dataset: str, start: date, end: date, chunk_symbols: int) -> dict:
    ...
    """
    from cn_market_lake.steps.intraday import resolve_scope
...
def derive(name: str, config_path: str, full: bool, start_str: str | None, end_str: str | None):
    ...
    if name == "adj_factors":
        ...
        from cn_market_lake.derive.trading_status_history import derive_suspension_history
        ...
        from cn_market_lake.derive.sector_routing import derive_sector_routing

        summary = derive_sector_routing(cfg)
        click.echo(json.dumps(summary, indent=2, default=str))
    elif name == "sector_code_map":
        from cn_market_lake.derive.sector_code_map import derive_sector_code_map
...
def verify(config_path: str, only: str | None, repair: bool, kinds: str | None):
    ...
    `cml audit` asks whether the data that landed is correct. This asks whether
    ...
    former.
    """
    from cn_market_lake.quality.verify import verify_lake
...
def status(config_path: str, show_datasets: bool):
    ...
    if show_datasets:
        ...
        if stale:
            click.echo(f"\n{stale} dataset(s) STALE — check runs with `cml status` / `cml retry`.")
            raise SystemExit(1)
        return
    ...
    if orphaned:
        ...
        summary["orphaned_note"] = (
            ...
            f">={int(cfg.batch_stale_seconds)}s — next cml run reconciles them; "
            "or `cml clean --reconcile-runs`"
        )
    click.echo(json.dumps(summary, indent=2, default=str))
...
@click.option(
    ...
    help=(
        ...
        "`cml retry` refetches them (data is refetched, not lost, but the retry "
        "becomes a full re-run). Do not use on success-without-compact runs — "
        "run `cml compact --run-id` first."
    ),
)
@click.option(
    "--reconcile-runs",
...
def stats_refresh(config_path: str, force: bool):
    ...
    """
    from cn_market_lake.storage.stats import refresh_stats_if_stale, stats_freshness
...
def stats_show(config_path: str, dataset: str | None, by_source: bool):
    """Summarise the stats tables. Run `cml stats rebuild` first."""
    ...
    if summary is None:
        raise click.ClickException("no stats yet — run `cml stats rebuild`")
    freshness = stats_freshness(cfg)
    ...
    stale_note = f"  STALE — {freshness.reason}; run `cml stats refresh`" if freshness.stale else ""
    click.echo(
...
def mcp_cmd(config_path: str, live: bool):
    ...
    the portable command and arguments are simply::
      ...
      cml mcp --config /path/to/cn-market-lake.toml
    ...
    Read-only, like `cml serve`. The tools query the lake; ingestion stays on
    the CLI, where a person runs it.
    """
    import logging
    import sys

    from cn_market_lake.mcp_server import serve_stdio
...
def _guard_mcp_data_root(cfg, config_path: str) -> None:
    ...
    raise click.ClickException(
        ...
        "If it is your lake and it is genuinely empty: `cml init` builds one, "
        "`cml demo` makes a 5-symbol sample in 30 seconds, and `--live` serves "
...
@click.option(
    ...
    "inside the lake, which is where `cml serve` reads it from.",
)
def sources(config_path: str, vantage: str, only: str | None, out: str | None):
    ...
    The report lands in the lake, and `cml serve` renders it at /source-health.
    ...
    """
    from cn_market_lake.diagnostics.source_health import STATUS_LABELS, ProbeStatus, run_probes
    ...
    click.echo(f"\nWrote {path}")
    click.echo("View it with: cml serve  \u2192  http://127.0.0.1:8787/source-health")
...
def servers(action: str, config_path: str):
    """Deprecated alias for `cml sources --only tdx_protocol`.
    ...
    """
    from cn_market_lake.diagnostics.source_health import PROBES_BY_KEY, ProbeStatus, run_probe

    click.echo("note: `cml servers test` is now `cml sources --only tdx_protocol`", err=True)
...
def delisted_discover(config_path: str, limit: int | None):
    ...
    from cn_market_lake.steps.delisted import discover_delisted
...
def delisted_repair(config_path: str, since: str | None):
    ...
    from cn_market_lake.steps.delisted import repair_delisted_instruments
    ...
    from cn_market_lake.steps.delisted import purge_subscription_placeholders
...
def delisted_backfill(config_path: str, since: str):
    ...
    from cn_market_lake.steps.delisted import backfill_delisted_bars
