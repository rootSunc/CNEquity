...
from cn_market_lake.domain.datasets import DATASETS, TIER_LABELS
...
def _read(config: Config, dataset: str, **kwargs: Any) -> pl.DataFrame:
    ...
    # auto-detection: the fallback resolves ./configs/cn-market-lake.toml from the
...
def _live_only(config: Config, tool: str, reason: str) -> None:
    ...
    raise ToolError(
        ...
        "Build one with `cml init` (or `cml demo` for a 5-symbol sample)."
    )
...
def _guard_sql(sql: str) -> str:
    ...
    if kind != duckdb.StatementType.SELECT:
        raise ToolError(
            ...
            "lake; ingestion and maintenance are `cml` commands."
        )
    return text
